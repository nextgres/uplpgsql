...
create function retc(int) returns two_int8s LANGUAGE uplpgsql as
$$ begin return row($1,1)::two_int8s; end $$;
select retc(42);

-- ok to return a matching record type
create or replace function retc(int) returns two_int8s LANGUAGE uplpgsql as
...
do LANGUAGE uplpgsql $$ declare c two_int8s; d nested_int8s;
begin
  c := row(1,2);
...
do LANGUAGE uplpgsql $$ <<b>> declare c two_int8s; d nested_int8s;
begin
  b.c := row(1,2);
...
-- error cases
do LANGUAGE uplpgsql $$ declare c two_int8s; begin c.x = 1; end $$;
do LANGUAGE uplpgsql $$ declare c nested_int8s; begin c.x = 1; end $$;
do LANGUAGE uplpgsql $$ declare c nested_int8s; begin c.x.q1 = 1; end $$;
do LANGUAGE uplpgsql $$ declare c nested_int8s; begin c.c2.x = 1; end $$;
do LANGUAGE uplpgsql $$ declare c nested_int8s; begin d.c2.x = 1; end $$;
do LANGUAGE uplpgsql $$ <<b>> declare c two_int8s; begin b.c.x = 1; end $$;
do LANGUAGE uplpgsql $$ <<b>> declare c nested_int8s; begin b.c.x = 1; end $$;
do LANGUAGE uplpgsql $$ <<b>> declare c nested_int8s; begin b.c.x.q1 = 1; end $$;
do LANGUAGE uplpgsql $$ <<b>> declare c nested_int8s; begin b.c.c2.x = 1; end $$;
do LANGUAGE uplpgsql $$ <<b>> declare c nested_int8s; begin b.d.c2.x = 1; end $$;

-- check passing composite result to another function
create function getq1(two_int8s) returns int8 LANGUAGE uplpgsql as $$
...
create function returnsrecord(int) returns record LANGUAGE uplpgsql as
...
create or replace function returnsrecord(int) returns record LANGUAGE uplpgsql as
...
create function getf1(x record) returns int LANGUAGE uplpgsql as
...
create function getf2(record) returns int LANGUAGE uplpgsql as
...
create function returnssetofholes() returns setof has_hole LANGUAGE uplpgsql as
$$
declare r record;
  h has_hole;
begin
...
create or replace function returnssetofholes() returns setof has_hole LANGUAGE uplpgsql as
$$
declare r record;
begin
...
create function sillyaddone(int) returns int LANGUAGE uplpgsql as
...
create function getf3(x mutable) returns int LANGUAGE uplpgsql as
...
create function sillyaddtwo(int) returns int LANGUAGE uplpgsql as
...
create function sillytrig() returns trigger LANGUAGE uplpgsql as
$$begin
...
create or replace function sillytrig() returns trigger LANGUAGE uplpgsql as
$$begin
  return row(new.*);
end$$;
...
create function compresult(int8) returns two_int8s LANGUAGE uplpgsql as
$$ declare r record; begin r := row($1,$1); return r; end $$;
